...
En Aplicaciones, AppStream es un enriquecimiento local secundario: la página y el catálogo curado aparecen primero y la lectura de AppStream empieza después. Una coincidencia fiable puede aportar icono y datos humanos a una elección libre; una coincidencia ambigua no reemplaza el nombre ni la descripción curados. Si AppStream falla o no tiene una ficha útil, la aplicación sigue visible y utilizable. Sunshine y Steam aparecen cada uno como una ficha propia con sus subopciones dentro; buscar sus nombres u opciones no debe caer por error en el resolvedor de nombres libres de Nixpkgs.
